...
| 김현지 | 도메인 아이디어 발굴부터 서비스 기획까지 주도하며, 전체 흐름 설계 후 세부 요구사항을 기능 단위로 구조화해 개발 가능한 형태로 정리했습니다. <br>구현 과정에서는 사용자/관리자/개발자 관점을 함께 고려해 로직과 정책을 설계하며 운영 가능한 서비스를 만드는 경험을 쌓았습니다. <br>사용자 관점에서는 예외 처리·입력 검증·실패/재시도 흐름을 정교화해 안정적인 사용성을 확보했습니다. <br>관리자 관점에서는 권한 분리와 조회/수정 범위를 명확히 해 운영 리스크를 줄였습니다. <br>개발자 관점에서는 인증/인가 및 민감 데이터 보호 등 보안 요소를 반영하고, DTO·계층 분리로 유지보수성과 확장성을 높였습니다. <br>결과적으로 기획을 시스템으로 번역하고 제품 관점에서 백엔드 아키텍처를 설계하는 역량을 강화했습니다. |
